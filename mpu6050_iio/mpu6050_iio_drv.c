...
static int mpu6050_i2c_write_reg(struct i2c_client *i2c, uint8_t reg_addr, uint8_t data)
{
    int ret = 0;
    uint8_t w_buf[2];
...
static int mpu6050_i2c_read_reg(struct i2c_client *i2c, uint8_t reg_addr, uint8_t *data)
{
    int ret = 0;
...
static int mpu6050_init(void)
{
    ...
    /* 解除休眠状态 */
    ret = mpu6050_i2c_write_reg(mpu6050_dev->i2c, MPU6050_PWR_MGMT_1, 0x00); /*  */

    /*  陀螺仪采样频率输出设置 */
    ret = mpu6050_i2c_write_reg(mpu6050_dev->i2c, MPU6050_SMPLRT_DIV, 0x07);

    ret = mpu6050_i2c_write_reg(mpu6050_dev->i2c, MPU6050_CONFIG, 0x06);

    /* 配置加速度传感器工作在 16G 模式, 不自检 */
    ret = mpu6050_i2c_write_reg(mpu6050_dev->i2c, MPU6050_ACCEL_CONFIG, 0x18);

    /* 陀螺仪自检及测量范围，典型值：0x18(不自检，2000deg/s) */
    ret = mpu6050_i2c_write_reg(mpu6050_dev->i2c, MPU6050_GYRO_CONFIG, 0x18);
#if MPU6050_USE_INT 
    /* 配置中断产生时中断引脚为低电平 */
    // ret = mpu6050_i2c_read_reg(mpu6050_dev->i2c, MPU6050_INT_PIN_CFG, &reg_val);
    // reg_val |= 0x80; 
    mpu6050_i2c_write_reg(mpu6050_dev->i2c, MPU6050_INT_PIN_CFG, 0x9C);

    /* 开启数据就绪中断 */
    ret = mpu6050_i2c_read_reg(mpu6050_dev->i2c, MPU6050_INT_ENABLE, &reg_val);
    reg_val |= 0x01;
    mpu6050_i2c_write_reg(mpu6050_dev->i2c, MPU6050_INT_ENABLE, reg_val);
#endif
    return ret;
}
...
static int mpu6050_data_ready_interrupt_on_off(uint8_t on_off)
{
    ...
    {
        reg_val |= 0x01;
        ret = mpu6050_i2c_write_reg(mpu6050_dev->i2c, MPU6050_INT_ENABLE, reg_val);
    }
    else
    {
        reg_val &= 0xFE;
...
static int mpu6050_deinit(void)
{
    int ret = 0;
    /* mpu6050复位, 寄存器恢复默认值 */
    ret = mpu6050_i2c_write_reg(mpu6050_dev->i2c, MPU6050_PWR_MGMT_1, 0x80);
    return ret;
}
...
static int mpu6050_read_id(uint8_t *id)
{
    ...
    ret = mpu6050_i2c_read_reg(mpu6050_dev->i2c, MPU6050_WHO_AM_I, &data);
...
static int mpu6050_read_accel(struct mpu6050_accel *acc)
{
    ...
    for (i = 0; i < 6; i++)
    {
        ret = mpu6050_i2c_read_reg(mpu6050_dev->i2c, MPU6050_ACCEL_XOUT_H+i, &data[i]);
    }
...
static int mpu6050_read_gyro(struct mpu6050_gyro *gyro)
{
    ...
    for (i = 0; i < 6; i++)
    {
        ret = mpu6050_i2c_read_reg(mpu6050_dev->i2c, MPU6050_GYRO_XOUT_H+i, &data[i]);
    }
...
static int mpu6050_read_temp(short *temp)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ret = mpu6050_i2c_read_reg(mpu6050_dev->i2c, MPU6050_TEMP_OUT_H+i, &data[i]);
    }
...
static irqreturn_t mpu6050_isr(int irq, void *dev)
{
    ...
    // mpu6050_i2c_read_reg(mpu6050_dev->i2c, MPU6050_INT_STATUS, &reg_val);
    // if (reg_val & 0x01)     
    // {
    //     /* 唤醒进程去读取数据 */
