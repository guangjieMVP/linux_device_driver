...
static int red_led_drv_open (struct inode *node, struct file *file)
{
    ...
    if (!atomic_dec_and_test(&led_dev.atomic_var))    //如果原子变量值减1小于0，说明在这一步操作前原子变量值已经为0
    {
        atomic_inc(&led_dev.atomic_var);              //atomic_dec_and_test已经将原子变量减1，将原子变量加1恢复为原来的值
        printk("failed to open led char device\n");
        return -EBUSY;
    }
...
{
    atomic_set(&led_dev.atomic_var, 1);      //释放时设置原子变量值为1
...
static int led_red_driver_probe(struct platform_device *dev)
{
    ...
    atomic_set(&led_dev.atomic_var, 1);     //设置原子变量为1
