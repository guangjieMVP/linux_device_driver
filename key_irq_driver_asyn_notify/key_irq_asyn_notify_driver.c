...
{
    return fasync_helper(fd, filp, mode, &g_keydev.async_queue);    
}

static int key_drv_release(struct inode *node, struct file *filp)
{
    /*  将文件从异步通知列表中删除  */
...
static void timeout_callback(struct timer_list *t)
{
    ...
    if (gpio_get_value(g_keydev.key_gpio))     //再次判断按键是否按下
    {
        ...
        my_key.is_press = 1;
           ...
           /* 异步通知 */
        if (g_keydev.async_queue)
        {
            kill_fasync(&g_keydev.async_queue, SIGIO, POLL_IN);      //发送IO信号, POLL_IN 标记有数据可读
