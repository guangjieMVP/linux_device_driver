...
static int red_led_drv_open (struct inode *node, struct file *file)
{
    ...
    mutex_lock(&led_dev.m_lock);    //枷锁
    printk("open mutex lock\n");
...
{   
    mutex_unlock(&led_dev.m_lock);
    printk("release mutex unlock\n");
    return 1;
}
...
static int led_red_driver_probe(struct platform_device *dev)  
{
    ...
    mutex_init(&led_dev.m_lock);    //初始化互斥锁
    printk("mutex init\n");
