...
static int red_led_drv_open (struct inode *node, struct file *file)
{
    ...
    spin_lock(&led_dev.spinlock);   //加锁
    if (led_dev.spinlock_count)     //设备已经被打开
    {
        printk("failed to open led device\n");
        spin_unlock(&led_dev.spinlock);   //解锁
        return -EBUSY;
    }
    led_dev.spinlock_count++;
    spin_unlock(&led_dev.spinlock);    //spinlock_count加一之后也要进行解锁
...
{
    spin_lock(&led_dev.spinlock);
    led_dev.spinlock_count--;           //释放后将计数变量减一
    spin_unlock(&led_dev.spinlock);

    printk("led release \n set value of atomic_var to 1 \n");
    return 1;
}
...
static int led_red_driver_probe(struct platform_device *dev)
{
    ...
    spin_lock_init(&led_dev.spinlock);     //初始化自旋锁变量
    led_dev.spinlock_count = 0;            //将计数初始化为零
